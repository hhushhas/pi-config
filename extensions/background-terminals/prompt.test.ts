...
test("start descriptions identify the shell and process-group containment boundary", () => {
  ...
  assert.match(BG_START_TOOL_DESCRIPTION, /owns the launched process group/);
  assert.match(BG_START_TOOL_DESCRIPTION, /taskkill \/T/);
  assert.match(BG_START_TOOL_DESCRIPTION, /Do not daemonize.*setsid/);
  assert.match(BG_START_TOOL_DESCRIPTION, /cannot be managed by bg_kill, \/stop, or session teardown/);
  assert.ok(BG_START_PROMPT_GUIDELINES.some((line) => /Never daemonize or detach/.test(line)));
  assert.doesNotMatch(BG_KILL_TOOL_DESCRIPTION, /whole process tree/);
});
