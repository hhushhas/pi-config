/**
 * Adapted from davis7dotsh/my-pi-setup at
 * d8534d7e6ec6609b7e684a8a0eb2e7a0195115ba for pi-config's session gates.
 *
 ...
 * - bg_kill: terminate the launched process group; returns final state.
 *
...
export const BACKGROUND_TERMINAL_STATE_TYPE = "background-terminal-state";
export const BACKGROUND_TERMINAL_TOOLS = [
  "bg_start",
  "bg_status",
  "bg_list",
  "bg_kill",
...
const BACKGROUND_TERMINAL_TOOL_SET = new Set<string>(BACKGROUND_TERMINAL_TOOLS);

export function activeToolsForBackgroundTerminals(
  activeTools: readonly string[],
  enabled: boolean,
): string[] {
  const next = activeTools.filter((name) => !BACKGROUND_TERMINAL_TOOL_SET.has(name));
  if (enabled) next.push(...BACKGROUND_TERMINAL_TOOLS);
  return [...new Set(next)];
}

export function restoreBackgroundTerminalEnabled(ctx: ExtensionContext): boolean {
  let enabled = true;
  for (const entry of ctx.sessionManager.getBranch()) {
    if (entry.type !== "custom" || entry.customType !== BACKGROUND_TERMINAL_STATE_TYPE) continue;
    const value = (entry.data as BackgroundTerminalState | undefined)?.enabled;
    if (typeof value === "boolean") enabled = value;
  }
  return enabled;
}

export async function stopRunningTerminals(
  manager: TerminalManagerShape,
  terminalRuntime: TerminalRuntime,
): Promise<ReadonlyArray<KillResult>> {
  const ids = manager.view
    .list()
    .filter((snap) => snap.status === "running")
    .map((snap) => snap.id);
  if (ids.length === 0) return [];
  return runTool(terminalRuntime, manager.kill(ids));
}
...
export default function (pi: ExtensionAPI) {
  ...
  const applyToolVisibility = (ctx?: ExtensionContext) => {
    pi.setActiveTools(activeToolsForBackgroundTerminals(pi.getActiveTools(), enabled));
    ctx?.ui.setStatus("background-terminal-gate", enabled ? undefined : "background terminals: off");
  };

  const setEnabled = (next: boolean, ctx: ExtensionContext) => {
    enabled = next;
    applyToolVisibility(ctx);
    pi.appendEntry<BackgroundTerminalState>(BACKGROUND_TERMINAL_STATE_TYPE, { enabled });
    ctx.ui.notify(
      enabled
        ? "Background terminal tools enabled for this session."
        : "Background terminal tools disabled for this session. Running terminals are unchanged; use /stop to terminate them.",
      "info",
    );
  };
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    enabled = restoreBackgroundTerminalEnabled(ctx);
    applyToolVisibility(ctx);
  });
  ...
  // the old extension instance. Disposing the runtime runs the manager
  // finalizer → disposeAll → every entry scope → bounded cleanup of Pi's
  // launched process group (taskkill /T on Windows). Commands must not escape
  // that ownership boundary by daemonizing into a new session.
  pi.on("session_shutdown", async (_event, ctx) => {
    sessionContext = undefined;
    ctx.ui.setStatus("background-terminal-gate", undefined);
  ...
  pi.registerCommand("enable-bg-terminal", {
    description: "Enable background-terminal tools for this session",
    handler: async (_args, ctx) => setEnabled(true, ctx),
  });

  pi.registerCommand("disable-bg-terminal", {
    description: "Disable background-terminal tools for this session",
    handler: async (_args, ctx) => setEnabled(false, ctx),
  });

  pi.registerCommand("stop", {
    description: "Stop all running background terminals",
    handler: async (_args, ctx) => {
      const manager = await getManager();
      const report = await stopRunningTerminals(manager, getRuntime());
      resultDelivery.consume(report.map((entry) => entry.id));
      ctx.ui.notify(
        report.length === 0
          ? "No background terminals are running."
          : report.length === 1
            ? "Stopped 1 background terminal."
            : `Stopped ${report.length} background terminals.`,
        "info",
      );
    },
  });
