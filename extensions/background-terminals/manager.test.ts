...
test(
  "kill terminates ordinary descendants in the launched process group",
  { skip: process.platform === "win32" },
  async () => {
    await withManager(async (manager, runtime) => {
      ...
      assert.ok(
        ...
        "same-group grandchild is gone after process-group termination",
      );
...
test(
  "a shell exit with inherited pipes open settles naturally and reaps same-group descendants",
  { skip: process.platform === "win32" },
  async () => {
    await withManager(async (manager, runtime) => {
      const snap = await runTool(
      ...
      assert.ok(
        ...
        "entry settled after launched-process-group cleanup",
      );
...
test(
  "a redirected same-group descendant is dead at settlement and after disposal",
  { skip: process.platform === "win32" },
  async () => {
    const runtime = createTerminalRuntime();
    ...
    try {
      ...
      const snap = await runTool(
        runtime,
        manager.start({
          command: `sleep 60 >/dev/null 2>&1 & echo "child:$!"`,
          title: "redirected-child",
          cwd,
        }),
      );
      assert.ok(
        ...
        "redirected descendant pid was printed",
      );
      const match = /child:(\d+)/.exec(
        manager.view.get(snap.id)?.stdout.text ?? "",
      );
      ...
      const { snap: done } = await settlement(manager, snap.id);
      assert.equal(done.status, "done", "natural shell exit remains truthful");
      assert.equal(
        processGone(descendant),
        true,
        "redirected same-group descendant is already gone at authoritative settlement",
      );
      ...
      assert.equal(
        processGone(descendant),
        true,
        "disposal leaves the owned process group gone",
      );
    } finally {
      await runtime.dispose().catch(() => undefined);
      if (descendant !== undefined && !processGone(descendant)) {
        try {
          process.kill(descendant, "SIGKILL");
        } catch {
          // The deterministic cleanup target already exited.
        }
      }
    }
  },
...
test(
  ...
  async () => {
    await withManager(async (manager, runtime) => {
      ...
      assert.ok(await pollUntil(() => manager.view.get(snap.id)?.status === "done"));
...
test("runtime.dispose terminates the running launched process group without a settle hook", async () => {
  const runtime = createTerminalRuntime();
  const manager = await runtime.runPromise(TerminalManager);
...
test("spill piping applies observable backpressure and keeps the writable queue bounded", async () => {
  ...
  const sink = new Writable({
    highWaterMark: 8 * 1024,
    write(written, _encoding, callback) {
      spilled += written.length;
  ...
  let maxQueued = 0;
  (sink as Writable & { write: typeof sink.write }).write = ((...args: Parameters<typeof sink.write>) => {
    const accepted = originalWrite(...args);
    backpressureObserved ||= !accepted;
    maxQueued = Math.max(maxQueued, sink.writableLength);
    return accepted;
  }) as typeof sink.write;

  let captured = 0;
  pipeSpillWithBackpressure(source, sink, (text) => {
  ...
  assert.equal(captured, chunk.length * count);
  assert.equal(spilled, chunk.length * count, "slow spill received every byte");
  assert.equal(backpressureObserved, true, "destination.write returned false");
  assert.ok(
    maxQueued <= sink.writableHighWaterMark + chunk.length,
    `writable queue stayed bounded (${maxQueued} bytes)`,
  );
});
