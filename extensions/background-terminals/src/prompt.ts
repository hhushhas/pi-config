...
export const BG_START_TOOL_DESCRIPTION =
  ...
  "Pi owns the launched process group on POSIX and uses taskkill /T on Windows. Do not daemonize, call setsid(), or otherwise detach into a new session: detached processes leave terminal ownership and cannot be managed by bg_kill, /stop, or session teardown. " +
  `Terminals are session-scoped: Pi stops the owned process group when the session ends or reloads. Output shown to you is tail-truncated (stdout ${formatSize(STATUS_STDOUT_MAX)}, stderr ${formatSize(STATUS_STDERR_MAX)}); the full logs are captured to files and in the /ps viewer. ` +
...
export const BG_START_PROMPT_GUIDELINES = [
  "Use foreground bash for quick blocking commands. Use bg_start for servers, watchers, long builds, long test suites, and commands expected to approach or exceed the foreground bash default of three minutes.",
  "bg_start processes receive no stdin — never start a command that requires interactive input.",
  "Never daemonize or detach a bg_start command into a new session; it would leave Pi's owned process group and could not be managed by bg_kill, /stop, or session teardown.",
...
export const BG_KILL_TOOL_DESCRIPTION =
  "Stop one or more running background terminals by terminating Pi's launched process group (SIGTERM escalating to SIGKILL on POSIX; taskkill /T on Windows). Returns each terminal's final state; already-settled ids are reported as such.";
