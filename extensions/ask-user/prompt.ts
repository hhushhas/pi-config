...
export const ASK_USER_TOOL_DESCRIPTION =
  "Ask the user a single multiple-choice question (2-5 options). The user may select an option, attach a free-form note to it, write their own answer, or dismiss the question. Ask exactly one question per call.";

/** Adds ask_user's multiple-choice capability to the model's available-tools prompt. */
export const ASK_USER_PROMPT_SNIPPET =
  "Ask the user a multiple-choice question (2-5 options, optional notes, or a free-form answer)";
...
export function buildAskUserResultMessage(
  ...
  switch (outcome.kind) {
    ...
    case "selected": {
      const selection = `User selected option ${outcome.index}: ${outcome.answer}`;
      return outcome.note ? `${selection}\nUser note: ${outcome.note}` : selection;
    }
  }
}
