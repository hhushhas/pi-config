 ...
 * - Tab or n adds a note to a supplied option (Esc returns to the same option)
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      const result = await ctx.ui.custom<SelectionResult>(
        (tui, theme, _kb, done) => {
          let optionIndex = 0;
          let editMode: "custom" | "note" | null = null;
          ...
          editor.onSubmit = (value) => {
            ...
            if (trimmed && editMode === "custom") {
              done({ answer: trimmed, wasCustom: true });
              return;
            }
            if (trimmed && editMode === "note") {
          ...
          function addNote() {
            if (allOptions[optionIndex].isOther) return;
            editMode = "note";
            editor.setText("");
            refresh();
          }
          ...
          function handleInput(data: string) {
            ...
            if (
              ...
              (data.toLowerCase() === "n" && data.length === 1)
            ) {
              addNote();
              return;
            }
          ...
          function render(width: number): string[] {
            ...
            for (let i = 0; i < allOptions.length; i++) {
              ...
              if (selected || (opt.isOther && editMode === "custom")) {
            ...
            if (editMode) {
              ...
              add(
                theme.fg(
                  "dim",
                  ` ↑↓ or 1-${allOptions.length} select • Enter confirm • Tab/n add note • Esc dismiss`,
                ),
              );
            }
    ...
    renderResult(result, _options, theme, _context) {
      ...
      const note = details.note
        ? `\n  ${theme.fg("muted", `Note: ${details.note}`)}`
        : "";
      return new Text(
        theme.fg("success", "✓ ") + theme.fg("accent", display) + note,
        0,
        0,
      );
    },
