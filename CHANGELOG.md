...
- Replaced the macOS-only `/usr/bin/lockf` workflow lock with `proper-lockfile`, made directory durability handling Windows-safe, corrected Windows absolute-path and migration-backup handling, corrected `cmd.exe /s` verbatim quoting and forced Windows tree termination so inherited pipes cannot leak, and made process fixtures and POSIX-mode assertions portable in the cross-platform gate.
