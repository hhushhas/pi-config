...
- Replaced the macOS-only `/usr/bin/lockf` workflow lock with `proper-lockfile`, made directory durability handling Windows-safe, corrected Windows absolute-path and migration-backup handling, and made process fixtures and POSIX-mode assertions portable in the cross-platform gate.
